...
class Trajectoire():
    def __init__(self, param, move, initial_point, Solo = True):
        # Solo regroupe le programme de mouvement + param, move et tirette + gestion des points via Registre
        self.Solo = Solo
        self.param = param
        self.move = move
        ...
        if self.Solo:
            ...
            self.X_abs, self.Y_abs, self.Theta_abs = self.point.x, self.point.y, self.point.theta
    ...
    def process(self, point, revert = False):
        # Traitement
        ...
        FinalAngle = PostTreatment(self.Theta_abs).step([point.x, point.y, point.theta])
            # fin
...
if __name__ == "__main__":
    robot = Trajectoire(None, None, None)
